...
module.exports=function handler(req,res){
  ...
  if(req.method==='GET') return ok(res,suppliesData,'Fetched district supply inventories');
  if(req.method==='POST'){
    const {district,supplyCategory='Medical Oxygen',requestedUnits='5,000 Liters'}=req.body||{};
    const r={requisitionId:`REQ-${Math.floor(1000+Math.random()*9000)}`,district,supplyCategory,requestedUnits,priority:'CRITICAL',status:'APPROVED & DISPATCHED',loggedAt:new Date().toISOString()};
    return ok(res,r,`Emergency supply requisition logged for ${district}`,201);
  }
