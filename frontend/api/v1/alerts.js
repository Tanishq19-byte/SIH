...
let alertsData=[
  ...
  {id:'ALT-2026-002',category:'Flood risk',severity:'High',timeDisplay:'18 mins ago',location:'NH-10 Teesta Bridge Melli, South Sikkim',affectedVehicle:'SK-01-B-3091 (Vaccine Cold Chain)',affectedDeliveryId:'DEL-VAC-3091',recommendedAction:'Monitor river level. Pre-position at Jorethang bypass entry.',isRead:false,summary:'Teesta at 8.2m. Submersion risk if level exceeds 9.0m.'},
...
module.exports=function handler(req,res){
  ...
  if(req.method==='GET') return ok(res,alertsData,`Fetched ${alertsData.length} alerts`);
  if(req.method==='POST'){
    const {category,severity,location,recommendedAction,summary}=req.body||{};
    const a={id:`ALT-2026-${Math.floor(1000+Math.random()*9000)}`,category,severity,timeDisplay:'Just now',timestamp:new Date().toISOString(),location,affectedVehicle:req.body.affectedVehicle||'Unspecified',recommendedAction,isRead:false,summary,createdAt:new Date().toISOString()};
    alertsData.unshift(a);
    return ok(res,a,'Alert created',201);
  }
