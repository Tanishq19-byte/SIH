...
module.exports=function handler(req,res){
  ...
  if(id){
    ...
    if(!route) return res.status(404).json({success:false,message:`Route ${id} not found`});
    return ok(res,route,`Fetched route ${route.name}`);
  }
  return ok(res,routesData,`Fetched ${routesData.length} highway corridors`);
};
